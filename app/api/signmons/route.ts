...
function getReply(payload: SignmonsResponse) {
  ...
  if (typeof payload.reply === "string" && payload.reply.trim()) {
    return payload.reply.trim().slice(0, 4000);
  }
...
export async function POST(request: Request) {
  ...
  if (
    ...
    return Response.json({ error: "Please enter a message of 1,000 characters or fewer." }, { status: 400 });
  }
  ...
  if (!apiUrl || !integrationKey) {
    return Response.json(
      { error: "The automated assistant is temporarily unavailable. Please call or text 216-703-3183." },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    const result = await response.json().catch(() => ({})) as SignmonsResponse;
    if (!response.ok) {
    ...
    return Response.json(
      { error: "The assistant could not respond. Please call or text Eternity Mechanical Services at 216-703-3183." },
      { status: 502 },
    );
  } finally {
