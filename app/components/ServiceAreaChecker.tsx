...
export default function ServiceAreaChecker({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`service-area-checker${compact ? " checker-compact" : ""}`}>
    ...
    <div className={`checker-result ${result.status}`} id={compact ? "home-zip-result" : "area-zip-result"} role="status" aria-live="polite">
      ...
      {result.status === "covered" && <><p><strong>Yes—we serve this area.</strong><span>ZIP {result.zip} is listed for {formatCities(result.cities)}. Appointment timing is confirmed directly.</span></p><div><a href="https://eternityhvacr.com/#schedule">Request service</a><a href="tel:+12167033183">Call 216-703-3183</a></div></>}
      {result.status === "confirm" && <><p><strong>Let’s confirm availability.</strong><span>ZIP {result.zip} is not on the current published list, but service may still be available based on the job and schedule.</span></p><div><a href="tel:+12167033183">Call to confirm</a><a href="https://eternityhvacr.com/#schedule">Send service details</a></div></>}
    </div>
  </div>;
}
