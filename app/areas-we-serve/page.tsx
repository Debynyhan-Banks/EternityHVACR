...
export default function AreasWeServePage() {
  return <main>
    ...
    <section className="section area-section"><div className="section-head"><div><p className="kicker">Primary coverage</p><h2>Core service cities and ZIP codes</h2></div><p>These priority markets receive the strongest service-area and search coverage.</p></div><div className="area-grid">{priorityServiceAreas.map((area) => {
      const href = area.city === "Euclid" ? "/areas-we-serve/euclid-oh" : area.city === "Cleveland Heights" ? "/areas-we-serve/cleveland-heights-oh" : null;
      const card = <article><span>{area.region ?? "Core"}</span><h3>{area.city}</h3><p>{area.zips.join(" • ")}</p>{href && <b>{area.city === "Euclid" ? "View real Euclid project work" : "View Cleveland Heights service"} →</b>}</article>;
      return href ? <Link className="area-card-link" href={href} key={area.city} aria-label={`View HVAC service in ${area.city}, Ohio`}>{card}</Link> : <div key={area.city}>{card}</div>;
    })}</div></section>
