...
export default function AreasWeServePage() {
  return <main>
    ...
    <section className="section area-section"><div className="section-head"><div><p className="kicker">Primary coverage</p><h2>Core service cities and ZIP codes</h2></div><p>These priority markets receive the strongest service-area and search coverage.</p></div><div className="area-grid">{priorityServiceAreas.map((area) => <article key={area.city}><span>{area.region ?? "Core"}</span><h3>{area.city}</h3><p>{area.zips.join(" • ")}</p></article>)}</div></section>
    ...
    <section className="section area-note"><div><p className="kicker">Not sure whether your property is covered?</p><h2>Send the city, ZIP code and equipment details.</h2><p>Eternity will confirm normal service-area coverage and appointment availability directly.</p></div><div><a className="btn" href="/#schedule">Check service availability <span>→</span></a><a href="mailto:ben@eternityhvacr.com">Email ben@eternityhvacr.com</a></div></section>
    <SiteFooter />
  </main>;
}
