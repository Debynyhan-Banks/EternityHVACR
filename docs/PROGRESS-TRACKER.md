...
**Current status:** The public estimator now uses five visual scope selectors, instant approved planning prices, a clearer inclusion-and-price-factor result, scope-aware service-request and Ask Eternity handoffs, local service links, crawlable comparison content, methodology and reviewer context. Four installation scopes publish planning prices; commercial RTUs still require a custom diagnostic and load calculation. The private second-opinion workflow and Cleveland Heights page remain published.

**Next action:** Complete the remaining labeled second-opinion acceptance upload, verify the owner-only download, and retain cleanup evidence for the 30-day boundary.
...
- [x] Publish the approved pricing estimator with scope-specific baseline ranges, complete-system and cooling-only starting prices, and custom commercial RTU review
