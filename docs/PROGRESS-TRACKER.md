...
**Current status:** A validated estimator release candidate now uses five visual scope selectors, instant approved planning prices, a clearer inclusion-and-price-factor result, scope-aware service-request and Ask Eternity handoffs, local service links, crawlable comparison content, methodology and reviewer context. Four installation scopes have published planning prices; commercial RTUs still require a custom diagnostic and load calculation. The private second-opinion workflow and Cleveland Heights page remain published.

**Next action:** Review and approve the estimator pricing release for public deployment, verify the live calculation and structured data, then complete the remaining labeled second-opinion acceptance upload.
...
| 5 | Scope-based project estimator | In review | Public deployment approval | Five visual selectors update instantly; the result separates included work from price factors, links to the matching service, and carries the selected scope into the request form and Ask Eternity; crawlable comparison content, methodology, FAQs and structured data use the approved pricing rules |
...
- [ ] Publish the approved pricing estimator with scope-specific baseline ranges, complete-system and cooling-only starting prices, and custom commercial RTU review
